...
test("user is warned when /save-md is missing a name", async () => {
  ...
  try {
    ...
    assert.deepEqual(notifications, [{ message: "Usage: /save-md name", type: "warning" }]);
  } finally {
    await rm(cwd, { force: true, recursive: true });
  }
});
...
test("user cannot accidentally overwrite an existing Markdown file", async () => {
  ...
  try {
    ...
    assert.deepEqual(notifications, [{ message: `File already exists: ${path}`, type: "error" }]);
  } finally {
    await rm(cwd, { force: true, recursive: true });
  }
});
...
test("user is warned when there is no assistant response to save", async () => {
  ...
  try {
    ...
    assert.deepEqual(await readdir(cwd), []);
    assert.deepEqual(notifications, [{ message: "No assistant response to save", type: "warning" }]);
  } finally {
    await rm(cwd, { force: true, recursive: true });
  }
});
