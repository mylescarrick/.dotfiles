...
test("parseWebFetchToolParams rejects invalid boundary input", () => {
  ...
  assert.deepEqual(
    parseWebFetchToolParams({ format: "pdf", url: "https://example.com" }, testFetchSettings),
    {
      _tag: "err",
  ...
  assert.deepEqual(
    parseWebFetchToolParams({ depth: "auto", url: "https://example.com" }, testFetchSettings),
    {
      _tag: "err",
  ...
  assert.deepEqual(
    parseWebFetchToolParams({ timeout: "30", url: "https://example.com" }, testFetchSettings),
    {
      _tag: "err",
  ...
  const credentialedUrl = parseWebFetchToolParams(
...
test("parseWebFetchToolParams clamps timeout to supported bounds", () => {
  const low = parseWebFetchToolParams({ timeout: 0, url: "https://example.com" }, testFetchSettings);
  const high = parseWebFetchToolParams({ timeout: 999, url: "https://example.com" }, testFetchSettings);
