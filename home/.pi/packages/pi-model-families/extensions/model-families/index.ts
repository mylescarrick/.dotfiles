...
function readJson(path: string, ctx?: ExtensionContext): unknown | undefined {
  ...
  try {
    ...
    ctx?.ui.notify(
      `Model families: failed to parse ${path}: ${error instanceof Error ? error.message : String(error)}`,
      "warning"
    );
    return undefined;
  }
...
function authStatusLabel(status: ProviderAuthStatusShape): string {
  if (status.configured)
    return status.label
      ? `${status.source ?? "configured"}:${status.label}`
      : (status.source ?? "configured");
  if (status.source)
    return status.label ? `unconfigured ${status.source}:${status.label}` : `unconfigured ${status.source}`;
  return "missing";
}
...
function helpText(config: ModelFamiliesConfig): string {
  return [
    ...
    `Disabled: ${
      familyNames(config)
        .filter((name) => config.families[name]?.disabled)
        .join(", ") || "none"
    }`,
    `Roles: ${ROLES.join(", ")}`,
  ].join("\n");
...
export default function modelFamilies(pi: ExtensionAPI) {
  ...
  function setStatus(ctx: ExtensionContext, role?: Role): void {
    const modeLabel =
      routingMode === "locked" ? `locked:${lockedModelKey ?? currentModelKey(ctx)}` : `auto:${activeFamily}`;
  ...
  function showStatus(ctx: ExtensionContext): void {
    ...
    const manualLines =
      family && Object.keys(family.manualTargets ?? {}).length > 0
        ? Object.entries(family.manualTargets ?? {})
            .map(
              ([name, target]) =>
  ...
  async function useFamily(familyName: string, ctx: ExtensionContext): Promise<void> {
    ...
    if (family.disabled) {
      ctx.ui.notify(
        `Model families: ${familyName} is disabled. Set disabled=false in ${CONFIG_FILE} and reload to use it.`,
        "warning"
      );
      return;
    }
  ...
  async function queueRole(role: Role, prompt: string, ctx: ExtensionContext): Promise<void> {
    routingMode = "auto";
    lockedModelKey = undefined;
    nextRoute = { reason: `manual one-shot ${role}`, role };
    setStatus(ctx, role);
    persistState();
  ...
  async function useManualTarget(name: string, prompt: string, ctx: ExtensionContext): Promise<void> {
    ...
    if (!target) {
      ...
      ctx.ui.notify(
        `Model families: unknown manual target ${name || "(missing)"}; available: ${available.join(", ") || "none"}`,
        "warning"
      );
      return;
    }
    ...
    persistState();
    ctx.ui.notify(
      `Model families: locked to manual target ${activeFamily}:${name}; use /mf auto to return`,
      "info"
    );
  ...
  pi.on("thinking_level_select", () => {
    if (thinkingSetByExtension || selectedByExtension || Date.now() < ignoreThinkingSelectionsUntil) {
      // biome-ignore lint/complexity/noUselessReturn: the guard documents which selections are ignored.
      return;
    }
